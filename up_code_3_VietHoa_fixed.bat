...
:: ========================================================
:: TRÌNH QUẢN LÝ GIT v3.0
:: Combined best from v1.0 + v2.1 + Improvements
:: Author: Grok Assisted
:: Đã Việt hóa toàn bộ + sửa lỗi hiển thị tiếng Việt (UTF-8)
:: ========================================================
...
echo  2. Tải thay đổi từ GitHub
echo  3. Kiểm tra thay đổi trên GitHub
...
if errorlevel 1 (set "INTERNET=Ngoại tuyến") else (set "INTERNET=Trực tuyến")
...
if "%MSG%"=="" set "MSG=Cập nhật %date:~-4%-%date:~-7,2%-%date:~-10,2% %time:~0,5%"
...
for /f "delims=" %%b in ('git for-each-ref --format="%%(refname:short)" refs/heads/') do (
    ...
    if errorlevel 1 echo [Bỏ qua] Nhánh %%b không có upstream hoặc không thể fast-forward.
)
